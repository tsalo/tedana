...
from tedana import __version__, combine, decay, io, tv, utils
...
def _get_parser():
    ...
    decay_args.add_argument(
        "--fittype",
        dest="fittype",
        choices=["loglin", "curvefit", "loglin-weighted", "loglin-irls"],
        help=(
            ...
            "to the raw data. "
            '"loglin-weighted" is a weighted log-linear fit (Michalek and Mikl, 2025) that '
            "weights each echo by the squared measured signal to compensate for the noise boost "
            "the logarithm introduces at later echoes. "
            '"loglin-irls" uses iteratively reweighted least squares with the squared '
            "model-predicted signal as weights. "
        ),
        default="loglin",
    )
    decay_args.add_argument(
...
def t2smap_workflow(
    ...
    fittype : {'loglin', 'curvefit', 'loglin-weighted', 'loglin-irls'}, optional
        ...
        'loglin-weighted' is a weighted log-linear fit (Michalek and Mikl, 2025)
        that weights each echo by the squared measured signal to compensate for
        the noise boost the logarithm introduces at later echoes.
        'loglin-irls' uses iteratively reweighted least squares with the squared
        model-predicted signal as weights.
    ...
    if tv_l2_denoise:
        ...
        data_denoised_masked = tv.denoise_tv_l2(
